    ...
    try {
      ...
      const leaseTenant =
        (leaseTenants || [])
          .map((row: any) => ({
            ...row,
            leases: Array.isArray(row.leases) ? row.leases[0] : row.leases,
          }))
          .find((row: any) => row?.leases?.status === 'active') ??
        (leaseTenants || []).map((row: any) => ({
          ...row,
          leases: Array.isArray(row.leases) ? row.leases[0] : row.leases,
        }))[0];
      ...
      if (leaseTenant?.leases) {
        ...
        setStats([
          ...
          },
          {
            ...
            value: nextPayment ? `${nextPayment.amount_due}€` : 'A jour',
            trend: nextPayment
              ? `Dû le ${new Date(nextPayment.due_date).toLocaleDateString('fr-FR')}`
              : 'Aucun',
            up: !nextPayment,
            icon: 'cash',
          },
        ...
        if (paymentsData) {
          setPayments(
            paymentsData.map((p) => ({
              mois: new Date(p.due_date).toLocaleDateString('fr-FR', {
                month: 'long',
                year: 'numeric',
              }),
              date: p.paid_at
            ...
            >
              ...
              {activeLease ? (
                <>
                  ...
                  >
                    {activeLease.properties.address},{' '}
                  ...
                  >
                    {[
                      {
                        icon: 'resize-outline',
                        label: `${activeLease.properties.surface_m2} m²`,
                      },
                      {
                        icon: 'grid-outline',
                        label: `${activeLease.properties.rooms} Pièces`,
                      },
                      {
                        icon: 'bed-outline',
                        label: activeLease.properties.is_furnished
                          ? 'Meublé'
                          : 'Non meublé',
                      },
                      {
                        icon: 'cash-outline',
                        label: `${activeLease.rent_amount} €`,
                      },
                    ].map((badge) => (
                      <View
                        key={badge.label}
            ...
            >
              ...
              >
                <SectionHeader
                  ...
                  title="Mes incidents"
                  subtitle="Suivi de vos déclarations"
                />
                <Pressable
