...
import TypeBienSelect from './TypeBienSelect';
...
export type TYPE_PROPERTY_TYPES = (typeof PROPERTY_TYPES)[number];
...
export default function AjouterBienModal({
  ...
  const [propertyType, setPropertyType] =
    useState<TYPE_PROPERTY_TYPES | null>('Maison');
  ...
  useEffect(() => {
    if (editProperty && visible) {
      ...
      setPropertyType(
        (editProperty.property_type as TYPE_PROPERTY_TYPES) || 'Maison',
      );
  ...
  const handleSave = async () => {
    if (!address || !rent || !surface || !rooms) {
      Alert.alert(
        'Champs manquants',
        "Veuillez remplir l'adresse, le loyer, la superficie et le nombre de pièces.",
      );
      return;
    }
    ...
    try {
      ...
      if (editProperty && property) {
        const removedImages = initialRemoteImages.filter(
      ...
      const newLocalImages = selectedImages.filter(
        (uri) => !uri.startsWith('http://') && !uri.startsWith('https://'),
      );
      console.log(
        '[Upload] Total selectedImages:',
        selectedImages.length,
        'New local images:',
        newLocalImages.length,
      );
      if (newLocalImages.length > 0 && property) {
        ...
        if (uploadFailed > 0) {
          Alert.alert(
            'Photos',
            `${uploadSuccess} photo(s) envoyée(s), ${uploadFailed} échec(s).`,
          );
      ...
      Alert.alert(
        'Succès',
        editProperty
          ? 'Le logement a été modifié.'
          : 'Le logement a été ajouté.',
      );
      onSuccess();
      onClose();
        ...
        <View style={styles.modalContent}>
          <View style={styles.header}>
            <Text style={styles.headerTitle}>
              {editProperty ? 'Modifier le logement' : 'Ajouter un logement'}
            </Text>
                ...
                <View style={{ position: 'relative' }}>
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      ...
                      if (cleaned.length > 4) {
                        formatted =
                          formatted.slice(0, 5) + '/' + cleaned.slice(4, 8);
                      }
                ...
                <View style={styles.toggleRowSmall}>
                  <Pressable
                    style={[
                      ...
                      isFurnished === true && styles.toggleActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.toggleText,
                        isFurnished === true && styles.toggleActiveText,
                      ]}
                    >
                      Oui
                    </Text>
                  ...
                  <Pressable
                    style={[
                      styles.toggleButtonSmall,
                      isFurnished === false && styles.toggleActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.toggleText,
                        isFurnished === false && styles.toggleActiveText,
                      ]}
                    >
                      Non
                    </Text>
              ...
              <View style={styles.field}>
                ...
                <View style={styles.toggleRowSmall}>
                  <Pressable
                    style={[
                      styles.toggleButtonSmall,
                      hasElevator === true && styles.toggleActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.toggleText,
                        hasElevator === true && styles.toggleActiveText,
                      ]}
                    >
                      Oui
                    </Text>
                  ...
                  <Pressable
                    style={[
                      styles.toggleButtonSmall,
                      hasElevator === false && styles.toggleActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.toggleText,
                        hasElevator === false && styles.toggleActiveText,
                      ]}
                    >
                      Non
                    </Text>
            ...
            <View style={[styles.field, { marginBottom: 16, zIndex: 10 }]}>
              ...
              >
                <Text style={styles.selectButtonText}>
                  {energyClass || 'Sélectionner'}
                </Text>
                <Ionicons
                  name={showEnergyClassPicker ? 'chevron-up' : 'chevron-down'}
                  size={18}
                  color="#64748b"
                />
              </Pressable>
