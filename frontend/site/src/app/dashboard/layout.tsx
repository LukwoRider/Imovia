            ...
            <main className="flex-1 md:ml-64 flex flex-col min-h-0 overflow-hidden">
                ...
                <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-20">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            {/* Dynamic Greeting based on path (Mock) */}
                            <h1 className="text-xl font-bold text-[#12182C]">
                                {usePathname()?.startsWith("/dashboard/owner") ? "Bonjour, Pierre !" : "Bonjour, David !"}
                            </h1>
                            <p className="text-sm text-slate-500 hidden sm:block">
                                {usePathname()?.startsWith("/dashboard/owner") ? "Espace Propriétaire Imovia" : "Bienvenue sur votre espace locataire Imovia"}
                            </p>
                        </div>
                    </div>
...
function ProfileLink() {
    const pathname = usePathname()
    const isOwner = pathname?.startsWith("/dashboard/owner")
    const href = isOwner ? "/dashboard/owner/profile" : "/dashboard/tenant/profile"
